...
Six ways to build the same thing. They share no code, no components and no build order — the only
thing they have in common is that all the personal content sits in one or two data files.
...
| 04 | [reconos](../apps/reconos) | React 19 + Vite 8 + React Router 7 | Tailwind v4, Recharts | Security operations console | Medium-High | Security and backend roles; the most conventional "app" of the six |
...
Payload is the gzipped JS + CSS in `dist/` after `npm run build`. Source is hand-written lines
only — `.ts`, `.tsx`, `.js`, `.css`, `.html`, excluding `node_modules` and `dist`.
...
| | bios | circuitos | net-topology | reconos | workspace-os | manga-book |
| --- | :-: | :-: | :-: | :-: | :-: | :-: |
| Build step required | — | ✓ | ✓ | ✓ | ✓ | ✓ |
| TypeScript | — | ✓ | ✓ | ✓ | ✓ | ✓ |
| Client-side router | — | — | — | ✓ | — | — |
...
want a `node_modules` folder in your life. It is the only one that is just files.
...
- `bios`, `circuitos`, `net-topology` and `reconos` are keyboard reachable and render as normal DOM,
  so screen readers and text scaling behave.
